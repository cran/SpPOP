...
#'         has been generated following U(0,2) and the regression coefficients are generated as linear function of locations.
#' @examples
...
SpPOP_linear4<-function(N,k,xlat,ylong) {
  ...
  B0<-(2*Latitude+Longitude)/6
  B1<-(Latitude/3)

  # spatially varying model for generating the dataset
  # Relation between Y and X is non-linear in the model
  Y_sim<-B0+exp(B1*X)+ e
