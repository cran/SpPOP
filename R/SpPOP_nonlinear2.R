...
#'         has been generated following U(0,1) and the regression coefficients are generated as non-linear function of latitudes and longitudes.
#' @examples
#' sp_nonlinear2<-SpPOP_nonlinear2(100,10,c(1:10),c(1:10))
#' @references
#'
...
SpPOP_nonlinear2<-function(N,k,xlat,ylong) {
  ...
  B0<- 2*sin(pi*(Latitude+Longitude)/6)
  B1<-(1/2)*exp(Latitude+Longitude)
